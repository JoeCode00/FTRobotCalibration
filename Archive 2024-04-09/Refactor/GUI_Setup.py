...
import numpy as np
# from GUI import button_send_to_robot
...
def show(UDPRobotClientSocket, RobotServerAddressPort, RobotServerBufferSize):
    ...
    with dpg.window(label="Robot Control", width=850, height=1000, pos=(0, 25), tag="Robot Control"):
        ColumnWidth = dpg.get_item_width('Robot Control')/3*0.95
        ...
        with dpg.collapsing_header(label="Buttons", default_open =True):
            with dpg.group(horizontal=True, height=RowHeight):
                dpg.add_button(label="ESTOP", tag='ESTOP', callback=grand_central_dispatch, width=200)
        ...
        with dpg.collapsing_header(label="Position Input/Current", default_open =True):
            with dpg.group(horizontal=True, height=RowHeight):
                with dpg.child_window(width=ColumnWidth):
                    VectorHelper(VectorName="PX", Units="mm", default_value=800, min_value=-1000, max_value=1000, min_delta=-10, max_delta=10, UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort)
                with dpg.child_window(width=ColumnWidth):
                    VectorHelper(VectorName="PY", Units="mm", default_value=800, min_value=-1000, max_value=1000, min_delta=-10, max_delta=10, UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort)
                with dpg.child_window(width=ColumnWidth):
                    VectorHelper(VectorName="PZ", Units="mm", default_value=180, min_value=-1000, max_value=1000, min_delta=-10, max_delta=10, UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort)
                    
        with dpg.collapsing_header(label="FTX Input/Current", default_open =True):
            with dpg.group(horizontal=True, height=RowHeight):
                with dpg.child_window(width=ColumnWidth):
                    VectorHelper(VectorName="FTX X", Units=None, default_value=0.4082, min_value=-1, max_value=1, min_delta=-0.1, max_delta=0.1, UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort)
                with dpg.child_window(width=ColumnWidth):
                    VectorHelper(VectorName="FTX Y", Units=None, default_value=0.8165, min_value=-1, max_value=1, min_delta=-0.1, max_delta=0.1, UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort)
                with dpg.child_window(width=ColumnWidth):
                    VectorHelper(VectorName="FTX Z", Units=None, default_value=-0.4082, min_value=-1, max_value=1, min_delta=-0.1, max_delta=0.1, UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort)
                    
        with dpg.collapsing_header(label="FTZ Input/Current", default_open =True):
            with dpg.group(horizontal=True, height=RowHeight):
                with dpg.child_window(width=ColumnWidth):
                    VectorHelper(VectorName="FTZ X", Units=None, default_value=-0.5774, min_value=-1, max_value=1, min_delta=-0.1, max_delta=0.1, UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort)
                with dpg.child_window(width=ColumnWidth):
                    VectorHelper(VectorName="FTZ Y", Units=None, default_value=0.5774, min_value=-1, max_value=1, min_delta=-0.1, max_delta=0.1, UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort)
                with dpg.child_window(width=ColumnWidth):
                    VectorHelper(VectorName="FTZ Z", Units=None, default_value=0.5774, min_value=-1, max_value=1, min_delta=-0.1, max_delta=0.1, UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort)
    ...
    with dpg.window(label="Sampling Control", width=875, height=700, pos=(0, 350), tag="Sampling Control"):
        ...
        with dpg.collapsing_header(label="Sampling Controls", default_open =True):
            with dpg.group(horizontal=True, height=RowHeight):
                dpg.add_button(label="Set Position \nAs Safe\nOrigin", tag='Sampling Set Safe Origin', callback=lambda:grand_central_dispatch('Sampling Set Safe Origin',None,None,UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort, RobotServerBufferSize=RobotServerBufferSize), width=dpg.get_item_width('Sampling Control')/4)
                with dpg.group():
                    with dpg.group(horizontal=True):
                        dpg.add_button(label="Immediate Z\nMove Up", tag='Immediate Z Move Up', callback=lambda:grand_central_dispatch('Immediate Z Move Up',None,None,UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort, RobotServerBufferSize=RobotServerBufferSize), width=150)
                        dpg.add_button(label="Immediate Z\nMove Down", tag='Immediate Z Move Down', callback=lambda:grand_central_dispatch('Immediate Z Move Down',None,None,UDPRobotClientSocket=UDPRobotClientSocket, RobotServerAddressPort=RobotServerAddressPort, RobotServerBufferSize=RobotServerBufferSize), width=150)
                    dpg.add_input_float(tag='Immediate Z Move Step', default_value=0.1, min_value=0, max_value=10,  format="%.2f", step=0.1, width=300)
                with dpg.group(horizontal=True, width=dpg.get_item_width('Sampling Control')/4):
                    with dpg.group():
                        dpg.add_text('Auto Decend')
                        dpg.add_radio_button(items=['False','True','Decending'], default_value='False',label="Auto Decend", tag='Auto Decend')
        ...
        with dpg.collapsing_header(label="Sampling Goto Buttons", tag="Sampling Goto Buttons", default_open =True):
            MakeSamplingButtons(SafePositionGridAndNames, UDPRobotClientSocket, RobotServerAddressPort, RobotServerBufferSize)
                        
                        
                 
    
    with dpg.window(label="Robot Angles (Command | Delta)", width=1000, height=250, pos=(850, 25), tag="Robot Angles"):
