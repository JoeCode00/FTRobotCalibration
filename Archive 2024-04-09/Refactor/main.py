...
def init_command_fig(HistorySeconds, FXYZLimit_N, TXYZLimit_Nm):
    ...
    AnglesSubfigTable = table.table(AnglesSubfigAxes, 
                        cellText = np.array(np.zeros((len(AngleTableRowLabels), len(AngleTableColLabels)))), 
...
def recieve_robot_data(UDPRobotClientSocket, RobotServerBufferSize):
...
def get_FT_data(SensorConnected, FTSocket, FTSocketAddress, FTPort, FTData):
    ...
    if SensorConnected:
        ...
        FX = np.sin(datetime.now().microsecond/1000000*2*np.pi)*10
        FY = np.sin(datetime.now().microsecond/1000000*2*np.pi+2*np.pi/3)*10
        FZ = np.sin(datetime.now().microsecond/1000000*2*np.pi+4*np.pi/3)*10
        TX = np.cos(datetime.now().microsecond/1000000*2*np.pi)
        TY = np.cos(datetime.now().microsecond/1000000*2*np.pi+2*np.pi/3)
        TZ = np.cos(datetime.now().microsecond/1000000*2*np.pi+4*np.pi/3)
...
def process_FT_data(NewFTData, FTData, SensorConnected, CountsPerForceAndTorque):
    ...
    FTData = np.append(FTData, np.zeros((1,FTData.shape[1],FTData.shape[2])), axis = 0)
    ...
    for i in range(0,6,1):
        if SensorConnected:
            ...
            FTData[FTData.shape[0]-1,i+1,0] = list(struct.unpack('>ffffff',NewFTData))[i]
    return FTData
...
def inverse_kinematics(v_FPtool, v_FS6, v_Fa67):#0   1   2   3   4   5   6
    Frames = pd.DataFrame(data = {'LinkLength': [0,         700,    900,    0,      0,      0,      0],     #a12 a23 a34 a45 a56 a67 a71
                               'TwistAngle': [3*np.pi/2,    0,      0,      3*np.pi/2, np.pi/2,   np.pi/2,   0],     #α12 α23 α34 α45 α56 α67 α71
...
def main():
    ...
    DAQInterval_ms = np.round(1000 / 100)  # [ms]
    ChartDrawInterval_ms = np.round(1000 / 50)  # [ms]
    ...
    FTData = np.array(np.zeros((1,7,2)), dtype = object)
    FTData[0,0,0] = datetime.now()
    
    v_FPtoolOld    = np.array([800,        800,        180])
    v_FS6Old       = np.array([-0.5774,    0.5774,    0.5774])
    v_Fa67Old      = np.array([ 0.4082,    0.8165,    -0.4082])
    ...
    try:
        ...
        while QuitBool == False:
            ...
            DataRecievedBool, DataRecievedList = recieve_robot_data(UDPRobotClientSocket, RobotServerBufferSize)
            ...
            if not(np.array_equal(v_FPtool, v_FPtoolOld)) or (
                    not(np.array_equal(v_FS6, v_FS6Old))) or (
                    not(np.array_equal(v_Fa67, v_Fa67Old))) or (
                ...
                if JoingAngleCellRefreshing == False:               
                    ...
                    JointAngleDeg = np.rad2deg(JointAngle[:,0:6])
                ...
                CellValue = np.round(JointAngleDeg[JointAngleCellRow, JointAngleCellCol], 2)
