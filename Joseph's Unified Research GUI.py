...
def init_command_fig(HistorySeconds, FXYZLimit_N, TXYZLimit_Nm):
    ...
    FigGridSpec = gridspec.GridSpec(
        2, 2, figure=CommandFig, width_ratios=[1, 1], height_ratios=[2, 1]
    )
    ...
    for i in arange(SCol0Nslider):
        SCol0AxesArray[i] = ButtonSliderSubfig.add_axes(
            (
                [
                    SCol0XOffsetFromLeftWall,
                    1 - (SCol0Spacing * (i + 1)),
                    SCol0Width,
                    SCol0Hight,
                ]
        ...
        # min value, max value, initial value, step value
    SCol1Info = [
        [-1000, 1000, 800, 1.0, "Position Vector X"],
    ...
    for i in arange(SCol1Nslider):
        SCol1AxesArray[i] = ButtonSliderSubfig.add_axes(
            (
                [
                    SCol1XOffsetFromLeftWall,
                    1 - (SCol1Spacing * (i + 1)),
                    SCol1Width,
                    SCol1Hight,
                ]
    ...
    (HistoryLineTX,) = HistoryTXYZ.plot(
        0, 0, label="TX", color="darkred", animated=True
    )
    (HistoryLineTY,) = HistoryTXYZ.plot(
        0, 0, label="TY", color="darkgreen", animated=True
    )
    (HistoryLineTZ,) = HistoryTXYZ.plot(
        0, 0, label="TZ", color="darkblue", animated=True
    )
    ...
    (PositionLineFXYZ,) = PositionFXYZ.plot(
        0, 0, label="PositionFXYZ", color="Orange", animated=True
    )
    ...
    (PositionLineTXYZ,) = PositionTXYZ.plot(
        0, 0, label="PositionTXYZ", color="Blue", animated=True
    )
...
def CreateButtons(ButtonSliderSubfig, ButtonTitle, ButtonXOffsetFromLeftWall):
    ...
    for i in arange(NButton):
        ButtonAxesArray[i] = ButtonSliderSubfig.add_axes(
            (
                [
                    ButtonXOffsetFromLeftWall,
                    1 - (ButtonSpacing * (i + 1)),
                    ButtonWidth,
                    ButtonHeight,
                ]
...
def process_FT_data(NewFTData, FTData, SensorConnected, CountsPerForceAndTorque):
    ...
    for i in range(0, 6, 1):
        if SensorConnected:
            FTData[FTData.shape[0] - 1, i + 1, 0] = (
                int.from_bytes(NewFTData[12 + i * 4 : 15 + i * 4], "big", signed=True)
                / CountsPerForceAndTorque
                * 256
            )
        else:

            FTData[FTData.shape[0] - 1, i + 1, 0] = list(unpack(">ffffff", NewFTData))[
                i
            ]
    return FTData
    ...
    for Row in range(FTData.shape[0] - 1, 0, -1):
        ...
        if TimeGap < timedelta(seconds=HistorySeconds):
            if FTDataRecentHistoryIsNew:
                ...
                # AddTimeGap = append()
                FTDataRecentHistory = append(
                    FTDataRecentHistory, array(zeros((1, 8, 2)), dtype=object), axis=0
                )
                FTDataRecentHistory[FTDataRecentHistory.shape[0] - 1, 0:7, :] = FTData[
                    Row, :, :
                ]
                FTDataRecentHistory[FTDataRecentHistory.shape[0] - 1, 7, :] = (
                    -TimeGap.total_seconds()
                )
...
def theta2(Choice, JointAngle, Frames, K1, K2):
    ...
    elif round(theta2b, 5) == round(theta2c, 5) or round(theta2b, 5) == round(
        theta2d, 5
    ):
...
def inverse_kinematics(v_FPtool, v_FS6, v_Fa67):  # 0   1   2   3   4   5   6
    Frames = DataFrame(
        data={
            "LinkLength": [0, 700, 900, 0, 0, 0, 0],  # a12 a23 a34 a45 a56 a67 a71
            "TwistAngle": [
                3 * pi / 2,
                0,
                0,
            ...
            "JointOffset": [0, 0, 0, 98, 145, 152.4, 0],
        }
    ...
    v_FP6orig = (
        v_FPtool
        - dot(v_6Ptool, v_i) * v_Fa67
        - cross(dot(v_6Ptool, v_j) * v_FS6, v_Fa67)
        - dot(v_6Ptool, v_k) * v_FS6
    )
    # S7
    ...
    for Choice in range(8):
        ...
        s12 = -1
        X17, Y17, Z17 = XYZ17(
            Choice, JointAngle, X7, Y7, Z7, c12, s12, c67, s67, c71, s71, gamma1
        )
        ...
        c56 = 0
        s56 = 1
        X671, Y671, Z671 = XYZ671(
            Choice, JointAngle, c56, s56, c67, s67, c71, s71, c12, s12, gamma1
        )
...
def main():
    ...
    (
        ...
        BCol3Array,
        QuitButton,
    ) = init_command_fig(HistorySeconds, FXYZLimit_N, TXYZLimit_Nm)
    ...
    try:
        ...
        while QuitBool == False:
            ...
            (
                ...
                FTXS,
                RobotBytesToSend,
            ) = update_command_variables(SCol0Array, SCol1Array, CommandFig)
            ...
            NewFTData = get_FT_data(
                SensorConnected, FTSocket, FTSocketAddress, FTPort, FTData
            )
            FTData = process_FT_data(
                NewFTData, FTData, SensorConnected, CountsPerForceAndTorque
            )
            FTDataRecentHistory = recent_FT_data_chunk(FTData, HistorySeconds)

            if FTDataRecentHistory is not None:
                HistoryLineFX.set_data(
                    FTDataRecentHistory[:, 7, 0], FTDataRecentHistory[:, 1, 0]
                )
                HistoryLineFY.set_data(
                    FTDataRecentHistory[:, 7, 0], FTDataRecentHistory[:, 2, 0]
                )
                HistoryLineFZ.set_data(
                    FTDataRecentHistory[:, 7, 0], FTDataRecentHistory[:, 3, 0]
                )
                HistoryLineTX.set_data(
                    FTDataRecentHistory[:, 7, 0], FTDataRecentHistory[:, 4, 0]
                )
                HistoryLineTY.set_data(
                    FTDataRecentHistory[:, 7, 0], FTDataRecentHistory[:, 5, 0]
                )
                HistoryLineTZ.set_data(
                    FTDataRecentHistory[:, 7, 0], FTDataRecentHistory[:, 6, 0]
                )

            PositionLineFXYZ.set_data(
                FTDataRecentHistory[:, 1, 0], FTDataRecentHistory[:, 2, 0]
            )
            PositionLineTXYZ.set_data(
                FTDataRecentHistory[:, 4, 0], FTDataRecentHistory[:, 5, 0]
            )
            ...
            if (
                not (array_equal(v_FPtool, v_FPtoolOld))
                or (not (array_equal(v_FS6, v_FS6Old)))
                or (not (array_equal(v_Fa67, v_Fa67Old)))
                or (JointAngleFirst == True)
                or (JoingAngleCellRefreshing == True)
            ):
                ...
                Cell = AnglesSubfigTable.properties()["celld"][
                    JointAngleCellRow + 1, JointAngleCellCol
                ]
                CellValue = round(
                    JointAngleDeg[JointAngleCellRow, JointAngleCellCol], 2
                )
